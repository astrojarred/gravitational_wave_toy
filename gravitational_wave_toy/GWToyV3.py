...
def observe_grb_parallel(params):
    """Modified version to increase timestep along with time size"""

    grb_file_path = params[0]
    sensitivity = params[1]
    log_directory = params[2]
    start_time = params[3]
    zeniths = params[4]
    sites = params[5]
    random_seed = params[6]
    target_precision = params[7]
    max_time = None
    energy_limits = [30, 10000]
    read = False
...
def batch_observe_grb(batch):
    batch_results = []
    for grb_params in batch:
        res = observe_grb_parallel(grb_params)
...
def run():
    ...
    results = pd.DataFrame(results_bag)
